...
#
# Usage: ./initpost.sh [options] <post name> <post date>
#
# Options:
...
#
# Example:
#   ./initpost.sh -c How to replace strings with sed 2016 10 17
#
# Important Notes:
...
# ----------------------------------------------------------------
POST_TITLE="${@:2:$(($#-4))}"
POST_NAME="$(echo ${@:2:$(($#-4))} | sed -e 's/ /-/g' | sed "y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/")"
DATE="$(echo ${@:$(($#-2)):$(($#))} | sed 's/\([0-9][0-9]\)/\1 /g;s/\(^..\) /\1/;s/ $//')"
CURRENT_DATE="$(date -j -f '%Y %m %d' "${DATE}" +'%F')"
...
# Set your blog URL
BLOG_URL="https://cupsmethods.github.io"
